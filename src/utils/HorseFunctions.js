...
const normalizePartners = (partners) => {
  ...
  const unique = Array.from(
    new Set(partners.filter((partnerId) => partnerId != null))
  );
  return unique
    .map((partnerId) => Number(partnerId))
    .filter((partnerId) => !Number.isNaN(partnerId));
};
...
const addPartnerToHorse = (members, horseId, partnerId) => {
  ...
  const targetIndex = members.findIndex(
    (member) => Number(member.id) === targetId
  );
  if (targetIndex === -1) {
    throw new Error("Caballo seleccionado no encontrado");
  }
  const partnerIndex = members.findIndex(
    (member) => Number(member.id) === partnerTargetId
  );
  if (partnerIndex === -1) {
...
const removeHorse = (members, horseId) => {
  ...
  return filtered.map((member) => {
    const parent1 =
      member.parent1Id != null && Number(member.parent1Id) === targetId
        ? null
        : member.parent1Id;
    const parent2 =
      member.parent2Id != null && Number(member.parent2Id) === targetId
        ? null
        : member.parent2Id;
    const partners = normalizePartners(member.partners).filter(
